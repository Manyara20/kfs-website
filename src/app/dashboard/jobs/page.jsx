...
export default function Jobs() {
  ...
  const handleAddOrUpdate = async (e) => {
    ...
    try {
      let response;
      if (editingJobId) {
        response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/jobs/${editingJobId}`, form, {
          headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        });
        setJobs(jobs.map((job) => (job.id === editingJobId ? response.data : job)));
        alert("Job updated successfully!");
      } else {
        response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/jobs`, form, {
          headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        });
  ...
  const handleArchive = async (id, currentArchivedStatus) => {
    ...
    try {
      const response = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/jobs/${id}/archive`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
